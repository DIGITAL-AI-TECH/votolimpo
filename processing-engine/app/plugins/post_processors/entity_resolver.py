...
class EntityResolver:
        ...
        async with pool.acquire() as conn:
            ...
            for pol in output.get("politicians", []):
                ...
                politician_id = await self._resolve_politician(
                    ...
                    politician_table, fuzzy_threshold, fuzzy_party_threshold,
                )
            ...
            for ent in output.get("entities", []):
                ...
                entity_id = await self._resolve_entity(
                    conn, name, ent_type, entity_table, entity_fuzzy_threshold,
                )
    ...
    async def _resolve_politician(
        ...
        party: str | None, state: str | None,
        table: str, fuzzy_threshold: float, fuzzy_party_threshold: float,
    ) -> int:
        ...
        row = await conn.fetchrow(
            f"SELECT id FROM {table} WHERE lower(name) = lower($1)", name.strip(),
        )
        if row:
            return row["id"]
        ...
        if party:
            ...
            """, normalized, fuzzy_party_threshold, party)
            if row:
                return row["id"]
    ...
    async def _resolve_entity(
        self, conn: asyncpg.Connection, name: str, ent_type: str,
        table: str, fuzzy_threshold: float,
    ) -> int:
