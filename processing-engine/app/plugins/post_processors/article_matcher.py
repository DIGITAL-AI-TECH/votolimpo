            ...
            for cand in candidates:
                sim = _calculate_similarity(
                    ...
                    },
                    {
                        ...
                        "politician_ids": [
                            x for x in (cand["politician_ids"] or []) if x
                        ],
                    },
                    weights,
                )
...
def _calculate_similarity(a: dict, b: dict, weights: dict) -> dict:
    ...
    entity_overlap = (
        len(pols_a & pols_b) / len(pols_a | pols_b) if pols_a | pols_b else 0.0
    )
    ...
    if pub_a and pub_b:
        ...
        temporal_prox = (
            1.0 if days <= 1 else 0.7 if days <= 7 else 0.3 if days <= 30 else 0.0
        )
