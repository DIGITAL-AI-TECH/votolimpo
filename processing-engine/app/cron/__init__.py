...
async def cron_recalculate_scores():
    ...
    async with pool.acquire() as conn:
        pids = [r["politician_id"] for r in await conn.fetch(
            "SELECT DISTINCT politician_id FROM votolimpo.politician_articles")]

        updated = 0
        for pid in pids:
            articles = [dict(r) for r in await conn.fetch("""
                SELECT a.severity::text, pa.role::text, a.veracity_score, a.published_at
            ...
            await conn.execute("""
                ...
                WHERE id = $4
            """, result["score"], json.dumps(result["components"]), len(articles), pid)

            await conn.execute("""
                INSERT INTO votolimpo.score_history (politician_id, score, components)
                VALUES ($1, $2, $3)
            """, pid, result["score"], json.dumps(result["components"]))
            updated += 1
...
async def cron_cleanup_logs():
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM votolimpo.processing_logs WHERE created_at < NOW() - INTERVAL '90 days'")
