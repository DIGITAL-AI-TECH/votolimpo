...
async def cron_refresh_mvs():
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "REFRESH MATERIALIZED VIEW CONCURRENTLY votolimpo.mv_article_relations"
        )
        await conn.execute(
            "REFRESH MATERIALIZED VIEW CONCURRENTLY votolimpo.mv_global_stats"
        )
        ...
        try:
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Você é um redator de enciclopédia política brasileira. "
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Você é um analista político imparcial. "
                    ...
                    {
                        "role": "user",
                        "content": f"Resuma {pol['name']} ({pol['party'] or '?'}/{pol['state'] or '?'}).\n\n{articles_text}",
                    },
                ],
                temperature=0.3,
                max_tokens=300,
...
def setup_cron_scheduler() -> AsyncIOScheduler:
    ...
    _scheduler.add_job(
        cron_deactivate_stale_clusters,
