...
def _cfg(content_type: str = "", max_chars: int | None = None) -> dict:
    """Build a config dict matching Ingestor Protocol."""
    cfg: dict = {}
    if content_type:
        cfg["content_type"] = content_type
    if max_chars is not None:
        cfg["max_content_chars"] = max_chars
    return cfg
...
class TestTextIngestor:
    ...
    async def test_passthrough_str(self):
        result = await self.ingestor.ingest("hello world", _cfg(CONTENT_TYPE_PLAIN))
        assert result == "hello world"

    async def test_passthrough_bytes_utf8(self):
        raw = "olá mundo".encode()
        result = await self.ingestor.ingest(raw, _cfg(CONTENT_TYPE_PLAIN))
    ...
    async def test_truncation_preserves_start_and_end(self):
        ...
        content = "A" * 100 + "B" * 100
        result = await self.ingestor.ingest(content, _cfg(max_chars=50))
    ...
    async def test_no_truncation_when_short(self):
        content = "short text"
        result = await self.ingestor.ingest(content, _cfg(max_chars=10000))
        assert result == "short text"

    async def test_truncation_exact_boundary(self):
        content = "x" * 100
        result = await self.ingestor.ingest(content, _cfg(max_chars=100))
...
class TestHTMLIngestor:
    ...
    async def test_strips_script_tags(self):
        ...
        result = await self.ingestor.ingest(html, _cfg(CONTENT_TYPE_HTML))
    ...
    async def test_truncation_applied(self):
        ...
        result = await self.ingestor.ingest(html, _cfg(max_chars=500))
...
class TestPDFIngestor:
    ...
    async def test_rejects_str_input(self):
        ...
        with pytest.raises(ValueError, match="expects bytes"):
            await self.ingestor.ingest("not bytes", _cfg(CONTENT_TYPE_PDF))
    ...
    async def test_invalid_bytes_raises_value_error(self):
        ...
        with pytest.raises(ValueError, match="Failed to open PDF"):
            await self.ingestor.ingest(b"not a pdf", _cfg(CONTENT_TYPE_PDF))
    ...
    async def test_valid_pdf_extraction(self):
        ...
        result = await self.ingestor.ingest(pdf_bytes, _cfg(CONTENT_TYPE_PDF))
    ...
    async def test_pdf_truncation(self):
        ...
        result = await self.ingestor.ingest(pdf_bytes, _cfg(max_chars=500))
        assert len(result) <= 500 + len("\n\n[...truncated...]\n\n")
...
class TestJSONIngestor:
    ...
    async def test_valid_json_serialized(self):
        ...
        raw = json.dumps(data)
        result = await self.ingestor.ingest(raw, _cfg(CONTENT_TYPE_JSON))
    ...
    async def test_invalid_json_raises(self):
        with pytest.raises(ValueError, match="Invalid JSON"):
            await self.ingestor.ingest("{not valid json}", _cfg(CONTENT_TYPE_JSON))
    ...
    async def test_truncation_applied(self):
        big = {"data": "x" * 10000}
        raw = json.dumps(big)
        result = await self.ingestor.ingest(raw, _cfg(max_chars=500))
        assert "[...truncated...]" in result
...
class TestAutoIngestor:
    ...
    async def test_routes_html_by_mime(self):
        ...
        result = await self.ingestor.ingest(html, {"content_type": "text/html"})
        assert "Hello" in result

    async def test_routes_json_by_mime(self):
        data = '{"key": "value"}'
        result = await self.ingestor.ingest(data, {"content_type": "application/json"})
        parsed = json.loads(result)
        assert parsed["key"] == "value"

    async def test_routes_text_by_mime(self):
        result = await self.ingestor.ingest("plain text", {"content_type": "text/plain"})
        assert result == "plain text"

    async def test_routes_text_by_default(self):
        result = await self.ingestor.ingest("unknown", {"content_type": "application/octet-stream"})
        assert result == "unknown"

    async def test_sniffs_html_from_bytes(self):
        html = b"<html><body><p>Sniffed HTML</p></body></html>"
        result = await self.ingestor.ingest(html, {"content_type": ""})
